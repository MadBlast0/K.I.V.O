...
- [x] **BENCH-05** · M0 · `vad` / `aec` suite: detection latency, false triggers with TTS playing, echo leakage into STT (§1) → done: `apps/kivo-bench/src/speech/vad_aec.rs` (Silero VAD v6 + WebRTC AEC3 via sonora on a simulated room: onset and end latency, false triggers and words leaked into STT from KIVO's own echo with and without AEC, ERLE, barge-in onset) · verified: saved run (2026-09-24): onset 290 ms, end 400 ms (incl. the 250 ms pause), echo alone without AEC 5.1 false triggers/min and 187 leaked words/min, with AEC3 0 and 0, ERLE 36 dB, user-over-KIVO onset 360 ms · note: the OS echo cancellation on real devices is VOICE-30's
...
- [x] **BENCH-09** · M3 · `brain` suite: TTFT, tokens/s, tool-call latency, structured-output validity, cancellation latency, error rate (§1) → done: `kivo-bench brain` (`apps/kivo-bench/src/suites/brain.rs`) on any brain named by `KIVO_BENCH_BRAIN` — a CLI agent over ACP or an OpenAI-compatible server: time to first token, short answer complete, output speed (400-word story), structured-output validity (JSON-only answer), cancel → stream ended, error rate, and for API brains prompt → tool call with one tool · verified: saved run (2026-09-24) on OpenCode's free model over ACP: first token 1.8 s p50, answer 2.5 s, cancel 76 ms, 0% errors, 100% valid JSON, 56 tokens/s; `structured_answers_are_checked` · note: tool-call latency is measured for API brains; an ACP agent runs its own tools
