...
- [~] **BENCH-09** · M3 · `brain` suite: TTFT, tokens/s, tool-call latency, structured-output validity, cancellation latency, error rate (§1) → partial: the brain path records its spans (t6Intent, t7FirstToken, t9FirstAudio, t10Complete) in the turn metrics · missing: the `brain` suite itself, deferred by the owner with the other benchmark runs (DECISIONS "Benchmarks deferred")
