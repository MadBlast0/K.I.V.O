...
- [x] **MEM-09** · M8 · Embeddings (MiniLM-class via `fastembed`/`ort`, 384-dim, shared with the intent router) in `sqlite-vec`; model id stored per vector, background re-embed on change (§3) → done: MiniLM-L6 (384-d, the intent router's model, shared) embeds each note into sqlite-vec 0.1.9 (`memory_vectors`, cosine), the model id kept per vector (`memory_vector_models`, profile-scoped); a changed note loses its vector by trigger and is embedded again in the background · verified: `notes_are_recalled_by_meaning_with_the_local_model`, store vector tests, the profile-scoping check
