...
4. Never typing into password fields. Each tool that can type checks this before the permission
   engine runs (`Tool::hard_limit`), so no one is asked to approve it, and again when it runs.
...
3. **Destination binding:** outbound actions (send, post, upload, HTTP to a non-allowlisted host)
   ...
   denied. A destination that first appears after untrusted content entered the turn is always
   refused; a link the brain knew on a clean turn may be opened, but nothing that sends data may
   go to it (DECISIONS "Destination binding, precisely").
...
- [x] **SEC-08** · M4 · Grants scoped by tool, argument pattern and duration (once / session / 24 h / always) in `permissions_grants`; the user can view and revoke them (§2) → done: grants by tool, target, `*` argument pattern and duration (this session, 24 h, always; migration 7), chosen on the card's "Always for…"; Permissions → Mode lists them with how long they last and Revoke · verified: `grants_match_argument_patterns`, `grants_round_trip_and_arguments_are_summarized_short` (durations, session end), `grants_and_the_audit_log_are_readable_and_revocable`, Permissions and Island tests (2026-09-23)
- [x] **SEC-09** · M5 · Task grants: a background task cannot exceed the permissions granted at creation (§2) → done: a task runs only its creation grants (exact arguments) in every mode, `NotGranted` otherwise · verified: `tasks_keep_to_their_grants_and_run_independent_steps_together`, `routines_run_only_what_was_granted`, policy tests (2026-09-23)
...
- [x] **SEC-27** · M4 · Full effect: cancel all turns and tasks, stop TTS, kill tool Job Objects, `session/cancel` to ACP agents, stop input injection, pause background tasks, audit entry (§8) → done: the emergency stop pauses running tasks, which resume only when the user says so · verified: `timeouts_pauses_and_checks` (2026-09-23)
...
- [x] **SEC-30** · M4 · Native messaging host manifest allows only KIVO's extension ID; payload limits (§9) → done: the host manifest names only KIVO's extension (`chrome-extension://ohaefaipbiknncgdjdpkjgaiocpnlloc/`), the host re-checks the caller, needs the session token on the user-only pipe, and every frame is capped at 1 MiB (the extension caps its replies too) · verified: `the_manifest_allows_only_one_origin`, `registration_round_trips_under_a_test_key`, `a_connection_without_the_token_is_refused`, `flags_are_parsed_and_unknown_ones_refused` (2026-09-23)
...
- [x] **SEC-32** · M9 · Updates: minisign-signed manifests + Authenticode binaries, both verified before running the installer (§9) → done: an update's installer runs only after its minisign signature verifies against the key compiled into the runtime and, in a code-signed build, `WinVerifyTrust` (whole chain, revocation checked) names KIVO's publisher; a file that fails either is deleted · verified: `a_tampered_or_foreign_installer_is_refused_and_deleted` (altered file, other key, other publisher), `a_signed_program_names_its_publisher_and_an_altered_copy_doesnt` and `an_unsigned_program_has_no_signer` against real files (2026-09-24)
