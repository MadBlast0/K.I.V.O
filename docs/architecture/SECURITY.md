...
- [~] **SEC-27** · M4 · Full effect: cancel all turns and tasks, stop TTS, kill tool Job Objects, `session/cancel` to ACP agents, stop input injection, pause background tasks, audit entry (§8) → partial: the emergency stop cancels the turn (and with it any agent prompt, which sends `session/cancel`), stops the voice, kills every running command's Job Object, raises the input abort flag, clears the in-use indicators and writes the audit entry · verified: `the_emergency_stop_reaches_commands`, `kill_all_ends_running_commands`, `cancelling_stops_every_layer_within_100_ms` · missing: cancelling and pausing background tasks, which arrive with tasks (M5)
