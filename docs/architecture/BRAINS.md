...
- **Reasoning level:** a `ModelRef` carries `reasoning: Off | Low | Medium | High` (none = the
  ...
  sent back through the tool loop), Gemini `thinkingBudget` (Pro can't turn it off). A CLI
  agent's models and reasoning values are its own (ACP `models` or `thought_level` config
  options), matched to these levels by name.
...
- [x] **BRAIN-11** · M8 · More API adapters: Groq, Mistral, DeepSeek, xAI and custom OpenAI-compatible endpoints (§4) → done: Groq, Mistral, DeepSeek and xAI from the catalog through the OpenAI-compatible adapter with their quirks (Mistral refuses `stream_options` with a 422, so KIVO leaves it out and reads usage from the last chunk; `reasoning_content` is never text; `max_tokens`), and custom endpoints (name + address, key tested before it's stored, a cloud brain for privacy) · verified: `contract_tests::groq_mistral_deepseek_and_xai_speak_the_compatible_dialect` (a mock that answers like Mistral's 422), `brain_turns::the_control_centers_brain_requests` (custom endpoint); no live call to these services (they need the owner's keys) (2026-09-24)
...
- [x] **BRAIN-16** · M6 · The KIVO MCP server is passed in each ACP session's MCP server list (§4) → done: `Agents::set_kivo_server` adds `kivo-runtime --mcp-server --agent <id>` to each ACP `session/new` mcpServers list for agents the user allowed (`apps/kivo-runtime/src/agents.rs`) · verified: `m6_extensions::kivos_mcp_server_shares_memory_only_with_allowed_agents` drives the server over MCP; `acp_tests::a_new_session_carries_kivos_mcp_server` checks the entry reaches the agent `session/new`
