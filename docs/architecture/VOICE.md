...
in Settings → Performance (the master switch, and the Graphics backend: Automatic, CUDA, Vulkan,
Metal or Processor only; Automatic is CUDA on NVIDIA once its runtime is downloaded, Vulkan on AMD
and Intel, Metal on a Mac): on a PC with a usable graphics card (≥ 3 GB of its own, not on
battery, not the low tier) it recommends whisper.cpp's Whisper on that backend (small for Recommended,
...
|---|---|---|---|
| `whisper_cpp.rs` | every Whisper (GGML), on CUDA, Vulkan or Metal | runtime adapter: whisper.cpp does the work | — (the pattern to follow) |
...
- [x] **VOICE-33** · M2 · Endpointing: Silero pause 250 ms + Smart Turn v3 (§3) → done: Silero pause 250 ms, then Smart Turn v3.2 decides (re-checked at 700 ms, end forced at 1.6 s) · verified: Smart Turn feature tests against the reference, `silence_after_speech_ends_the_utterance`, the spoken end-to-end tests (2026-09-23); a pause after KIVO's name alone ("Kivo, … mute") isn't taken for the end: the end-of-turn model is asked only once the partial transcript has more than the name (DECISIONS "A name and a pause isn't the end"; `the_name_alone_is_not_a_request`, `saying_mute_mutes_with_no_ai_and_a_brief_answer` with Smart Turn installed, 2026-09-26)
...
- [~] **VOICE-50** · M8 · Local models on the GPU the way desktop dictation apps (Handy) run them: GGML engines on Vulkan with the CPU as fallback, no DirectML. Text-to-speech moves to a GGML voice engine on Vulkan (evaluate TTS.cpp's Kokoro GGUF and OuteTTS through llama.cpp; licence, quality, first audio, cancel ≤ 100 ms); Parakeet's DirectML encoder path is removed (CPU, or a GGML port if one exists); a Graphics backend setting (Automatic: CUDA on NVIDIA, Vulkan on AMD/Intel, Metal on Mac; each selectable, plus Processor only) with the CUDA runtime downloaded on demand; the registry, recommendation and Voice page show the GPU engines first (plan and repositories: `docs/research/gpu-speech/REPORT.md`) (owner, 2026-09-25; DECISIONS "GGML on Vulkan, no DirectML") → partial: DirectML is removed (DECISIONS "DirectML removed"; every ONNX engine opens its session through `onnx::session`, CPU only). The Graphics backend setting (`performance.graphics-backend`, only the choices the PC has, Settings → Performance), the chosen card passed by name (`GpuTarget`) and found among ggml's devices (the wrong-GPU fix), `kivo-infer-cuda` (the worker's source built with CUDA 13.4) started for a CUDA target with NVIDIA's cuBLAS from the on-demand CUDA pack on its PATH, a crash on CUDA stepping down to Vulkan, `Accel::Metal` and any GPU backend counted as the GPU, device names in Voice → Advanced, and Supertonic's interruptible runs; whisper.cpp's stop callback is KIVO's own (whisper-rs 0.16's read garbage) (DECISIONS "VOICE-50: GPU backends") · verified (2026-09-26, owner's PC: RTX 3060 Laptop, driver 616.92, Radeon iGPU listed first): the real CUDA pack downloaded from NVIDIA through the model manager and checked; whisper.cpp base.en on CUDA through KIVO's engine, the card found by name, 107 ms vs 1.1 s on the processor, a cancel returns in 4.7 ms; the runtime starting the CUDA worker with only the pack's two DLLs and transcribing (`a_cuda_target_transcribes_on_the_cuda_worker_when_it_is_here`, 92 ms); Supertonic's cancel 3.7 ms (was 3.6 s); the GPU policy, backend choice, worker switching and recovery unit tests; GPU vendor and driver read on this PC; clippy clean; kivo-voice, kivo-store, kivo-runtime (lib), kivo-core, kivo-platform, kivo-ipc tests and the UI tests pass. Also (2026-09-26): whisper.cpp on Vulkan, each card chosen by name (RTX 3060 127 ms, cancel 39 ms; the Radeon iGPU found by its name too); every ONNX engine's real-model tests on the CPU build of ONNX Runtime after the DirectML removal (Moonshine, Parakeet, ONNX Whisper, Kokoro, Supertonic, Chatterbox, the wake-word spotter, CAM++, Smart Turn, MiniLM), their models installed through the real catalog manifests (`installs_real_catalog_models_when_asked`) · open: GPU voices (no GGML voice runtime runs on Windows' GPUs, REPORT step 4); Metal is built for macOS builds but untested (no Mac, and no macOS platform layer to list its GPU yet); the release workflow's CUDA build is verified on the first tag; whisper.cpp on the processor takes ~1 s to stop (whisper.cpp, upstream too, gives the stop callback only to its CPU-only helper, not the encoder and decoder; the voice still goes quiet at once)
