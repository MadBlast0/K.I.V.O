...
| [M0](#m0) | 🟡 In progress | 51 | 43 | 7 | 1 | 0 | 86% | 1/4 |
| [M1](#m1) | 🟡 In progress | 86 | 79 | 7 | 0 | 0 | 92% | 2/4 |
| [M2](#m2) | 🟡 In progress | 32 | 31 | 0 | 1 | 0 | 100% | 1/3 |
| [M3](#m3) | 🟡 In progress | 61 | 61 | 0 | 0 | 0 | 100% | 3/4 |
...
| [M8](#m8) | 🟡 In progress | 46 | 39 | 5 | 1 | 1 | 87% | 0/2 |
| [M9](#m9) | 🟡 In progress | 18 | 3 | 15 | 0 | 0 | 17% | 0/2 |
| [L1](#l1) | ⬜ Not started | 2 | 0 | 0 | 0 | 2 | 0% | — |
| [L2](#l2) | ⬜ Not started | 1 | 0 | 0 | 0 | 1 | 0% | — |
| [Post](#post) | ⬜ Not started | 16 | 0 | 0 | 0 | 16 | 0% | — |
| **Total** | | **459** | **399** | **37** | **3** | **20** | **88%** | |
<!-- /progress -->
...
<!-- items:M0 -->
**51 items** · ✅ 43 done · 🟡 7 partial · ⏭ 1 dropped · ⬜ 0 not started
...
- [x] **BENCH-03** · `tts` suite: first audio (short/medium/long), RTF, RAM, cancel-to-silence (§1) → done: `apps/kivo-bench/src/speech/tts.rs` (Kokoro-82M int8 through sherpa-onnx, 4 CPU threads: cold load, first audio for short/medium/long text, RTF, cancel → silence, memory) · verified: saved run on the owner's PC (2026-09-24), `docs/benchmarks/2026-09-24-amd-ryzen-7-6800h.md`: cold load 1.4 s, first audio 1.7 s (short) / 2.4–2.7 s, RTF 2.2 (short) → 1.3 (long), cancel → silence < 0.1 ms, +416 MB · note: this int8 Kokoro is slower than real time on this CPU; the in-app benchmark (BENCH-15) measures KIVO's own Kokoro <!-- synced:x -->
...
- [x] **BENCH-05** · `vad` / `aec` suite: detection latency, false triggers with TTS playing, echo leakage into STT (§1) → done: `apps/kivo-bench/src/speech/vad_aec.rs` (Silero VAD v6 + WebRTC AEC3 via sonora on a simulated room: onset and end latency, false triggers and words leaked into STT from KIVO's own echo with and without AEC, ERLE, barge-in onset) · verified: saved run (2026-09-24): onset 290 ms, end 400 ms (incl. the 250 ms pause), echo alone without AEC 5.1 false triggers/min and 187 leaked words/min, with AEC3 0 and 0, ERLE 36 dB, user-over-KIVO onset 360 ms · note: the OS echo cancellation on real devices is VOICE-30's <!-- synced:x -->
...
- [~] **BENCH-13** · CI smoke subset on every PR: intent-router latency, IPC round trip, state-machine and cancellation tests (§4) → partial: `kivo-bench router` (`suites/router.rs`: the bundled grammar routes a command to the fast path and a request on to a brain against 200 apps and 30 windows) joins `ipc` in `ci.yml`'s smoke step; the state-machine and cancellation tests are unit tests in the same job · verified: saved local run (2026-09-24): command → fast path 61 µs p50 (322 µs p95), request → on to a brain 4 µs; ipc ping 32 µs · missing: its first run in CI, on the first `v*` tag (Actions run only on tags, owner decision) <!-- synced:~ -->
...
- [x] **PLAN-13** · Test layers in place: unit tests (routing, permissions, state, config, adapters, task graph) from M0; integration tests as each subsystem lands; end-to-end tests for voice → done: unit tests in every crate from M0; integration tests per subsystem (brains, tools, tasks, MCP, memory, updater); end-to-end tests for voice on the real speech worker and Moonshine (`spoken_command`: speak → recognize → act → answer, wake word, barge-in with a speaker's echo, cancellation ≤ 100 ms, crash recovery), the M5/M7/M8 journeys on the scripted rig, and Playwright against the dev app · verified: `cargo test --workspace` and the UI suites pass (2026-09-24) <!-- synced:x -->
<!-- /items -->
...
<!-- items:M3 -->
**61 items** · ✅ 61 done · 🟡 0 partial · ⏭ 0 dropped · ⬜ 0 not started
...
- [x] **BENCH-09** · `brain` suite: TTFT, tokens/s, tool-call latency, structured-output validity, cancellation latency, error rate (§1) → done: `kivo-bench brain` (`apps/kivo-bench/src/suites/brain.rs`) on any brain named by `KIVO_BENCH_BRAIN` — a CLI agent over ACP or an OpenAI-compatible server: time to first token, short answer complete, output speed (400-word story), structured-output validity (JSON-only answer), cancel → stream ended, error rate, and for API brains prompt → tool call with one tool · verified: saved run (2026-09-24) on OpenCode's free model over ACP: first token 1.8 s p50, answer 2.5 s, cancel 76 ms, 0% errors, 100% valid JSON, 56 tokens/s; `structured_answers_are_checked` · note: tool-call latency is measured for API brains; an ACP agent runs its own tools <!-- synced:x -->
- [x] **BENCH-15** · "Benchmark this engine" in the Control Center: STT first-partial and final latency, RTF, WER, CPU/RAM/VRAM, noise; TTS first audio, RTF, CPU/RAM/VRAM, long text, interruption; stored locally and shown against KIVO's thresholds (VOICE §11) → done: "Benchmark this engine" on every installed local engine's card (Voice page, `SpeechChooser`): `voice.benchmark` runs the engine in a separate test worker (`Switcher::benchmark`) — a recognizer transcribes five commands a Windows voice speaks, clean and with hiss and hum about 10 dB under the speech (end of speech → final, RTF, word errors clean and noisy); a voice speaks a short text three times, a long one, and a long one cancelled at its first audio (first audio, RTF, cancel → silence); both report the worker's CPU share and memory (with GPU memory when it uses the GPU). Results are kept (`voice.engineBenchmarks`), replace older suite numbers in the registry and show against KIVO's thresholds (`kivo_voice::registry::thresholds`) with Meets / Over budget · verified: `an_engine_is_benchmarked_on_this_pc` (real worker: Moonshine Base 0.17× real time, 13.8% WER on the commands; Windows voices first audio 42 ms, cancel 56 ms), `an_engine_benchmark_replaces_an_older_suite_run`, the Voice page test (2026-09-24) · note: cloud engines aren't benchmarked (it would spend the user's quota) <!-- synced:x -->
...
- [x] **BRAIN-14** · CLI agents over ACP: Claude Code (`claude-agent-acp`), Gemini CLI (`--acp`) and Codex (`codex-acp`) (§4) → done: KIVO runs CLI agents over ACP with the catalog's entry points (Claude Code `claude-agent-acp`, Gemini CLI `--acp`, Codex `codex-acp`, OpenCode `acp`); starting and opening a session time out after 60 s with "it may need you to sign in" instead of holding every later agent request, and stopping an agent ends its whole process tree (npm agents run under a `cmd /c` shim) · verified: live (2026-09-24): Claude Code and Codex (Zed's adapters, installed) and OpenCode pass the handshake; OpenCode answered a real prompt (`KIVO_TEST_REAL_AGENT_PROMPTS=1`) and left no process behind; `an_agent_that_never_answers_fails_instead_of_hanging`; the whole agent turn with a scripted ACP agent · note: a prompt to Claude Code or Codex needs the owner's own plan on them (Claude Code isn't signed in on the command line; codex-acp 0.16 doesn't know the configured `gpt-5.6-terra`); both errors reach KIVO cleanly <!-- synced:x -->
...
10. **GPU speech like Handy.** GGML engines on Vulkan for text-to-speech too, no DirectML (VOICE-50).
...
- [ ] **M8-X1** · The plan §157 beta definition is fully met (PLAN-28). → open: PLAN-28 is met except the benchmark runs (deferred by the owner)
- [ ] **M8-X2** · No known critical crash after a week of daily use. → open (owner): a week of daily use can only come from the owner

### M8 items

<!-- items:M8 -->
**46 items** · ✅ 39 done · 🟡 5 partial · ⏭ 1 dropped · ⬜ 1 not started
...
#### [VOICE.md](architecture/VOICE.md) (7)
...
- [ ] **VOICE-50** · Local models on the GPU the way desktop dictation apps (Handy) run them: GGML engines on Vulkan with the CPU as fallback, no DirectML. Text-to-speech moves to a GGML voice engine on Vulkan (evaluate TTS.cpp's Kokoro GGUF and OuteTTS through llama.cpp; licence, quality, first audio, cancel ≤ 100 ms); Parakeet's DirectML encoder path is removed (CPU, or a GGML port if one exists); the registry, recommendation and Voice page show the GPU engines first (owner, 2026-09-25; DECISIONS "GGML on Vulkan, no DirectML") <!-- synced:  -->
...
<!-- items:M9 -->
**18 items** · ✅ 3 done · 🟡 15 partial · ⏭ 0 dropped · ⬜ 0 not started

> Generated by `pnpm docs:sync` from the specs. Mark items **in their spec** (with the `→` note), then re-run.

#### [BENCHMARKS.md](architecture/BENCHMARKS.md) (1)

- [~] **BENCH-14** · Full suites before each release on the reference machines; a > 10% regression on a budgeted metric blocks the release (§4) → partial: `pnpm bench:gate <previous>.json <this>.json` (`scripts/bench-gate.mjs`) compares two full runs from the same reference machine and blocks on any lower-is-better p50 more than 10% worse (every such metric is gated, a superset of the budgeted ones; 1 ms / 1 MB / 0.5-point noise floor) or on a metric not measured again · verified: `bench-gate.test.mjs` (2026-09-24) · missing: the full suites on the reference machines before a release (bench runs deferred by the owner) <!-- synced:~ -->

#### [DISTRIBUTION.md](architecture/DISTRIBUTION.md) (7)

- [~] **DIST-03** · Installer size under 40 MB, checked in the release pipeline (§1, BENCHMARKS §3) → partial: `release.yml` fails the build when the NSIS or MSI installer is over 40 MB (no models are bundled) · verified: the step reads the bundle folder the smoke test already uses (2026-09-24) · missing: the first tagged run of `release.yml` (tags only; the owner releases) <!-- synced:~ -->
- [~] **DIST-04** · MSI per-machine for IT, updater-aware (§1) → partial: the bundle builds the per-machine MSI beside the NSIS installer; the updater recognizes an MSI install (under Program Files), takes the manifest's `windows-<arch>-msi` installer and runs `msiexec /i … /passive /promptrestart AUTOLAUNCHAPP=True` · verified: `the_channel_and_install_kind_choose_the_manifest_and_installer`, `installers_run_as_the_updater_runs_them` (2026-09-24) · missing: an MSI built and installed (the first tagged run of `release.yml` (tags only; the owner releases)) <!-- synced:~ -->
- [~] **DIST-07** · `tauri-plugin-updater` with a minisign-signed `latest.json` per channel (Stable, Beta, Experimental), channel chosen in Settings (§2) → partial: the runtime is the updater (DECISIONS "M9 build"): channels Stable/Beta/Experimental (the installed build's by default, chosen in Settings → About), `<channel>.json` manifests in `tauri-plugin-updater`'s format written by `scripts/update-manifest.mjs` from the bundler's minisign signatures and published on the `updates` release, verified with `minisign-verify` against the key compiled in from `src-tauri/updater.pub` · verified: `an_update_is_found_verified_installed_and_proves_itself` and `a_tampered_or_foreign_installer_is_refused_and_deleted` (real minisign keys and signatures, a local server), `update-manifest.test.mjs`, Updates UI tests (2026-09-24) · missing: the owner's minisign key (`pnpm tauri signer generate`, public half as `updater.pub`, private in the `release` environment) and a published manifest <!-- synced:~ -->
- [~] **DIST-08** · Update flow: download + verify → ask (or install at idle if opted in) → runtime finishes/cancels turns and persists state → exits → installer `/UPDATE` → relaunch → health check (§2) → partial: download + verify → a notification (Install now / When idle) or install at idle when chosen (KIVO and the PC idle 10 min, no task running) → a running turn gets 10 s → the pending update is recorded → the installer starts detached (`/P /R /UPDATE`, relaunching KIVO) → the runtime quits → after the restart the runtime marks the update healthy after 15 s and What's new shows · verified: the updater tests with a recording launcher, `kivo_core::update` tests (2026-09-24) · missing: a real update from one released version to the next <!-- synced:~ -->
- [~] **DIST-09** · Rollback: if the runtime fails to start twice after an update, reinstall the previous version (kept for one version) (§2) → partial: before installing, the running version's installer is kept (or fetched and verified from its release); the app counts its starts of an unproven update's runtime, retries one that doesn't come up within 20 s, and after two failed starts runs the kept installer and exits; only the current and previous installers stay · verified: `two_failed_starts_roll_back_to_the_previous_installer_once`, `without_a_kept_installer_there_is_nothing_to_roll_back_to`, `an_update_that_never_installed_is_forgotten`, the kept-installer check in the updater test (2026-09-24) · missing: a forced failed update rolled back on a real install <!-- synced:~ -->
- [~] **DIST-11** · Authenticode signing (Azure Artifact Signing or OV certificate) of all three executables, the installers and the native-messaging host; betas published signed (§3) → partial: `scripts/sign.ps1` signs with Azure Artifact Signing or an OV certificate (SHA-256, timestamped) and is Tauri's `signCommand` for KIVO.exe, both sidecars (kivo-runtime is also the native-messaging host) and the installers when the owner's secrets are set; `release.yml` then checks every file with `Get-AuthenticodeSignature` · verified: the workflow and script read against Tauri's signCommand contract (2026-09-24) · missing: the owner's signing route and credentials, and the first tagged run of `release.yml` (tags only; the owner releases) <!-- synced:~ -->
- [~] **DIST-16** · Generated `THIRD_PARTY_NOTICES` (`cargo about` + npm license checker) in the installer and in About (§6) → partial: `pnpm licenses:gen` writes THIRD_PARTY_NOTICES.txt from the licence and notice files each shipped crate and npm package carries (700 packages, identical texts once, 1.3 MB; DECISIONS "M9 build"), bundled as a resource and opened from Settings → About → Licences → Full licence texts (`about.notices`, shipped `.txt`/`.md`/`.pdf` only) · verified: generated on this PC, `m7_settings` opens it, `only_shipped_documents_open` (2026-09-24) · missing: checked inside a built installer (the first tagged run of `release.yml` (tags only; the owner releases)) <!-- synced:~ -->

#### [RELEASE.md](architecture/RELEASE.md) (5)

- [~] **REL-07** · Full matrix: Windows ARM64, macOS universal `.dmg` + `.app.tar.gz`, Linux `.AppImage`/`.deb`/`.rpm`; mac/Linux built but kept as 14-day workflow artifacts, not published (§2, §3) → partial: `release.yml` builds Windows x64 and ARM64 (both published), macOS universal and Linux x64 (AppImage/deb/rpm) as 14-day workflow artifacts that are never published and whose failures don't block a release · verified: the workflow (2026-09-24) · missing: the first tagged run of `release.yml` (tags only; the owner releases); the macOS and Linux ports are post-M9, so those builds are expected to fail until then <!-- synced:~ -->
- [~] **REL-08** · Experimental channel from `vX.Y.Z-exp.N` tags through `release.yml` (artifacts expire after 14 days) (§2) → partial: `vX.Y.Z-exp.N` tags are the Experimental channel (`channelOf` in `update-manifest.mjs`), published as pre-releases with `experimental.json`, and Experimental releases older than 14 days are deleted on each publish · verified: `update-manifest.test.mjs` (2026-09-24) · missing: the first tagged run of `release.yml` (tags only; the owner releases) <!-- synced:~ -->
- [~] **REL-09** · Platform signing steps (Authenticode `signCommand`; later Apple notarization; Linux `SHA256SUMS`/GPG) (§2) → partial: Windows Authenticode through `signCommand` (DIST-11) and SHA256SUMS.txt for every release; Apple notarization and GPG wait for the ports · verified: the workflow (2026-09-24) · missing: the owner's signing credentials and the first tagged run of `release.yml` (tags only; the owner releases) <!-- synced:~ -->
- [~] **REL-10** · SBOM (`cargo cyclonedx` + npm) and `THIRD_PARTY_NOTICES` uploaded with each release (§2) → partial: the `sbom` job writes CycloneDX SBOMs for kivo-runtime, kivo-infer and kivo-app (`cargo cyclonedx`) and the app's npm packages (`cdxgen`), plus THIRD_PARTY_NOTICES.txt, all uploaded with the draft release · verified: the workflow (2026-09-24) · missing: the first tagged run of `release.yml` (tags only; the owner releases) <!-- synced:~ -->
- [~] **REL-11** · `release` environment secrets (`TAURI_SIGNING_PRIVATE_KEY` + password, signing credentials), release jobs only on tags from `main`, a `production` approval for Stable (§2, §4) → partial: the build jobs read the signing secrets from the `release` environment; a `check` job refuses a tag that isn't on main; the `publish` job waits for the `production` environment's approval for every channel · verified: the workflow (2026-09-24) · missing: the owner creating both environments, their secrets and the reviewers in the repository settings <!-- synced:~ -->

#### [SECURITY.md](architecture/SECURITY.md) (1)

- [x] **SEC-32** · Updates: minisign-signed manifests + Authenticode binaries, both verified before running the installer (§9) → done: an update's installer runs only after its minisign signature verifies against the key compiled into the runtime and, in a code-signed build, `WinVerifyTrust` (whole chain, revocation checked) names KIVO's publisher; a file that fails either is deleted · verified: `a_tampered_or_foreign_installer_is_refused_and_deleted` (altered file, other key, other publisher), `a_signed_program_names_its_publisher_and_an_altered_copy_doesnt` and `an_unsigned_program_has_no_signer` against real files (2026-09-24) <!-- synced:x -->

#### [UX.md](architecture/UX.md) (1)

- [x] **UX-59** · "What's new in KIVO x.y" dialog shown once after an update, with Release notes / Got it (mockup → System surfaces, DIST-08) → done: "What's new in KIVO x.y" (Installed just now, the release notes as a list, Release notes / Got it) shows once in the Control Center after an update proves itself (`installed.json`, `updates.seen`) · verified: `shows what's new once after an update`, `a_healthy_update_leaves_whats_new_and_two_installers`, the updater test's What's new check (2026-09-24) <!-- synced:x -->

#### [KIVO_Project_Plan.md](KIVO_Project_Plan.md) (3)

- [~] **PLAN-29** · Production definition (§158) fully met → partial: of §158, built and tested — the runtime's supervision and recovery, the security suite, the updater with rollback, diagnostics, broad provider support, computer control on the dummy app, performance profiles, accessibility, the specs and automated tests, and safe defaults · missing: evidence only use and releases give: a signed release updating itself in the field, the benchmark runs, a week of daily use <!-- synced:~ -->
- [~] **PLAN-30** · Quality gates (§159) pass for the release → partial: the gates that tests can show are green — no known permission bypass (security suite), provider failures don't crash KIVO, core Windows actions deterministic on the dummy app, interruption and cancellation · missing: no known critical crash after daily use, the idle budget and wake reliability measured (bench runs deferred), onboarding reviewed by the owner <!-- synced:~ -->
- [x] **PLAN-31** · Architecture invariants checklist (§160, ARCHITECTURE §8) all green → done: `pnpm invariants` (`scripts/invariants.mjs`) runs the tests that enforce each of the twelve invariants (ARCHITECTURE §8) and counts a skipped test as not proven; new checks for invariant 2 (`the_app_reaches_kivo_only_through_ipc`) and 10 (`Secret` has no Display, Serialize or Clone: compile-fail doc tests) · verified: all twelve green on this PC with the speech-model tests run (2026-09-24) <!-- synced:x -->
<!-- /items -->
