...
| [M1](#m1) | 🟡 In progress | 86 | 0 | 1 | 0 | 85 | 0% | 0/4 |
...
- [~] **ARCH-01** · `kivo-runtime` binary: starts, holds the `Local\KIVO.Runtime.<user-sid>` mutex (a second runtime exits), serves IPC (§1) → partial: `apps/kivo-runtime` stub prints its version · missing: mutex, IPC, everything else <!-- synced:~ -->
- [ ] **ARCH-02** · The runtime owns the tray (`tray-icon` on a runtime thread with a Win32 message loop) and it survives a UI crash (§1) <!-- synced:  -->
- [ ] **ARCH-03** · The runtime launches `kivo-app` and relaunches it with backoff if it crashes (§1) <!-- synced:  -->
- [~] **ARCH-04** · `KIVO.exe` (Tauri) connects to the runtime pipe; if absent it spawns `kivo-runtime.exe` (sidecar, `externalBin`) detached, then connects; it shows the runtime state and can restart without stopping KIVO (§1) → partial: `apps/kivo-app/src-tauri` shell builds and runs with the UI · missing: sidecar, pipe connection, runtime state <!-- synced:~ -->
- [ ] **ARCH-05** · `tauri-plugin-single-instance`: a second `KIVO.exe` forwards "show main window" and exits (§1) <!-- synced:  -->
- [ ] **ARCH-11** · `kivo-platform` defines every trait in the §2 table (`AudioIo`, `EchoCancel`, `Hotkeys`, `Tray`, `Apps`, `Windows`, `UiAutomation`, `Input`, `Screen`, `Ocr`, `Secrets`, `SystemInfo`, `Notifications`); core crates depend only on the traits (§2) <!-- synced:  -->
- [ ] **ARCH-12** · A `Capabilities` struct is filled at startup (Windows build, OS AEC, Mica, NPU, …); Windows 10 differences are handled only inside `kivo-platform-windows` (§2) <!-- synced:  -->
- [ ] **ARCH-13** · `kivo-testkit` provides fake platform, fake audio and fake brain implementations for tests (§7) <!-- synced:  -->
- [ ] **ARCH-14** · Named-pipe transport (`interprocess`), `\.\pipe\kivo-<user-sid>`, security descriptor granting only the current user's SID, `PIPE_REJECT_REMOTE_CLIENTS` (§3) <!-- synced:  -->
- [ ] **ARCH-15** · Random 256-bit session token in `%LOCALAPPDATA%\KIVO\run\session.token` (user-only ACL); clients must present it in `hello` (§3) <!-- synced:  -->
- [ ] **ARCH-16** · Length-prefixed frames carrying JSON-RPC 2.0: request/response, notifications (events), and a message size limit (§3, SECURITY §9) <!-- synced:  -->
- [ ] **ARCH-17** · `hello { protocol_version, client, capabilities }`; incompatible major versions are rejected with a clear error (§3) <!-- synced:  -->
- [ ] **ARCH-18** · Rust types in `kivo-ipc` are the source of truth; TypeScript types are generated (`specta` or `ts-rs`) into `apps/kivo-app/src/ipc/generated.ts`, and CI fails when they are stale (§3) <!-- synced:  -->
- [ ] **ARCH-19** · The UI reconnects with backoff and re-subscribes; on reconnect the runtime sends a full `StateSnapshot`, then deltas (§3) <!-- synced:  -->
- [ ] **ARCH-22** · `enum Event` in `kivo-core` with every group in §4.1 (Voice, Turn, Tool, Task, System incl. `FileChanged`/`BrowserChanged` from plan §66, Provider, Ui), each carrying `ts` (monotonic + wall), `turn_id`/`task_id` where relevant, and `trace_id`; delivered over `tokio::sync::broadcast` (§4.1) <!-- synced:  -->
- [ ] **ARCH-24** · The session state machine (Idle, Listening, Thinking, Acting, Speaking, FollowUp, Interrupted, Paused, AwaitingConfirmation, Error) with every transition in the §4.2 diagram, unit-tested; an invalid transition logs at `error` (§4.2) <!-- synced:  -->
- [ ] **ARCH-25** · A Turn has a root `CancellationToken`; child tokens go to STT, the brain request, each tool call and TTS (§4.2) <!-- synced:  -->
- [ ] **ARCH-29** · Config: `%APPDATA%\KIVO\config\kivo.toml` (+ `profiles/*.toml`) with `schema_version`; validated on load, migrated forward, previous version backed up; all §5 config sections exist (§5) <!-- synced:  -->
- [ ] **ARCH-30** · Database: `%LOCALAPPDATA%\KIVO\data\kivo.db`, rusqlite (bundled), WAL, embedded migrations (§5) <!-- synced:  -->
- [ ] **ARCH-31** · Every user-owned record carries `profile_id` from the first migration, so people profiles need no later migration (§5, UX §8.2) <!-- synced:  -->
- [ ] **ARCH-32** · Logs: `tracing` rolling JSON in `%LOCALAPPDATA%\KIVO\logs\`, 7-day retention, a redaction layer for keys, tokens and `sensitive` fields; transcripts logged only with the "debug transcripts" setting (§5, §6) <!-- synced:  -->
...
- [~] **ARCH-34** · Rust stable with MSRV pinned in `rust-toolchain.toml`; `cargo fmt`; `clippy -D warnings` → partial: toolchain file, `rustfmt.toml`, clippy runs with zero warnings locally · missing: `-D warnings` enforced in CI (REL-03) <!-- synced:~ -->
- [ ] **ARCH-35** · `cargo deny` configured: licenses (GPL/AGPL denied in the default graph), advisories, bans (§7) <!-- synced:  -->
- [~] **ARCH-36** · UI toolchain: Node LTS + pnpm, TypeScript strict, ESLint, Prettier, Vitest (§7) → partial: pnpm, TypeScript strict · missing: ESLint, Prettier, Vitest <!-- synced:~ -->
- [ ] **ARCH-39** · `kivo-app` has no access to the store, secrets or tools; its only API is IPC (reviewed in every milestone) (§8) <!-- synced:  -->

#### [BENCHMARKS.md](architecture/BENCHMARKS.md) (10)

- [~] **BENCH-01** · `kivo-bench` CLI with the harness: machine fingerprint (CPU, GPU, RAM, OS build, power source), ≥ 5 runs with p50/p95, warmup excluded from cold metrics, JSON into the `benchmarks` table and `bench-results/<date>-<machine>.json`, Markdown summary into `docs/benchmarks/` (§1) → partial: `apps/kivo-bench` stub · missing: everything <!-- synced:~ -->
- [ ] **BENCH-02** · `stt` suite: cold load, first partial, final latency, RTF, WER (overall + accented), peak RAM/VRAM, CPU% on LibriSpeech subset, accented English and 50 KIVO commands (§1) <!-- synced:  -->
- [ ] **BENCH-03** · `tts` suite: first audio (short/medium/long), RTF, RAM, cancel-to-silence (§1) <!-- synced:  -->
- [ ] **BENCH-04** · `wake` suite: false accepts/hour on ≥ 10 h negative audio, false rejects % on the recorded "Hey Kivo" set, CPU% (§1) <!-- synced:  -->
- [ ] **BENCH-05** · `vad` / `aec` suite: detection latency, false triggers with TTS playing, echo leakage into STT (§1) <!-- synced:  -->
- [ ] **BENCH-06** · `idle` suite: CPU%, RAM, wakeups/s and package power over 30 min with listening on (§1) <!-- synced:  -->
- [ ] **BENCH-07** · `overlay` suite: hotkey → first frame, white-flash check, GPU utilization and power idle vs animating (PresentMon) (§1) <!-- synced:  -->
- [ ] **BENCH-10** · Reference machines recorded: the owner's PC (mid/high) and the 4-core / 8 GB / no-GPU VM (low, labelled approximate) (§2, DECISIONS) <!-- synced:  -->
- [ ] **BENCH-11** · The first benchmark report is committed to `docs/benchmarks/` and the engine defaults in DECISIONS.md are updated with measured numbers (§1) <!-- synced:  -->
- [ ] **BENCH-13** · CI smoke subset on every PR: intent-router latency, IPC round trip, state-machine and cancellation tests (§4) <!-- synced:  -->

#### [DISTRIBUTION.md](architecture/DISTRIBUTION.md) (2)

- [ ] **DIST-10** · Config and database migrations run forward only, with a backup first (§2, ARCH-29) <!-- synced:  -->
- [ ] **DIST-17** · `cargo deny` denies GPL/AGPL in the default graph; GPL components only as separately downloaded add-ons after legal review (§6, ARCH-35) <!-- synced:  -->

#### [INTEGRATIONS_AND_PLUGINS.md](architecture/INTEGRATIONS_AND_PLUGINS.md) (1)

- [ ] **INT-11** · Nothing assumes the UI is local-only: IPC is versioned and clients are untrusted (§4) <!-- synced:  -->

#### [RELEASE.md](architecture/RELEASE.md) (5)

- [ ] **REL-01** · Conventional Commits, enforced by `pr-title.yml` (§1, §2) <!-- synced:  -->
- [ ] **REL-02** · `release-please` Release PR bumping `Cargo.toml` (workspace), `tauri.conf.json` and `package.json`, writing `CHANGELOG.md`; merging tags `vX.Y.Z` (§1) <!-- synced:  -->
- [ ] **REL-03** · `ci.yml` on every PR and push to `main`: Rust fmt, clippy `-D warnings`, tests and `cargo deny` on windows-latest, plus ubuntu-latest and macos-latest for the portable crates; UI install, typecheck, lint, Vitest; generated IPC types up to date; smoke benchmarks (§2, §3) <!-- synced:  -->
- [ ] **REL-04** · Required status checks added to the `main` ruleset once CI exists; Actions pinned by commit SHA (§4) <!-- synced:  -->
- [ ] **REL-12** · `codeql.yml` and Dependabot version updates (§2) <!-- synced:  -->

#### [SECURITY.md](architecture/SECURITY.md) (4)

- [ ] **SEC-18** · `Secret<String>` newtype: no `Display`, `Debug` prints `***`, no `Serialize`, zeroized on drop (§5) <!-- synced:  -->
- [~] **SEC-28** · Tauri: strict CSP, no remote content, `withGlobalTauri: false`, no `shell` plugin, minimal `capabilities/*.json` per window (the overlay window gets almost nothing) (§9) → partial: CSP set, `withGlobalTauri` off (default), no shell plugin, main-window capability limited to core + window controls · missing: overlay window and its capability file <!-- synced:~ -->
- [ ] **SEC-29** · IPC hardening: user-only DACL, reject remote clients, session token, schema validation, message size limit (§9, ARCH-14–16) <!-- synced:  -->
- [~] **SEC-31** · Dependencies: `cargo deny`, `pnpm audit`, lockfiles committed, Dependabot (§9) → partial: `Cargo.lock` and `pnpm-lock.yaml` committed, Dependabot alerts on · missing: `cargo deny`, `pnpm audit` in CI <!-- synced:~ -->

#### [UX.md](architecture/UX.md) (1)

- [ ] **UX-05** · Overlay spike: a transparent, borderless, non-focusable (`WS_EX_NOACTIVATE`), always-on-top, click-through window with no taskbar entry; white-flash and show latency measured (§2, BENCH-07) <!-- synced:  -->

#### [VOICE.md](architecture/VOICE.md) (1)

- [ ] **VOICE-01** · Capture through `AudioIo` (WASAPI via windows-rs, cpal fallback) and playback, measured for idle CPU in the audio spike (§1) <!-- synced:  -->

#### [KIVO_Project_Plan.md](KIVO_Project_Plan.md) (1)

- [ ] **PLAN-13** · Test layers in place: unit tests (routing, permissions, state, config, adapters, task graph) from M0; integration tests as each subsystem lands; end-to-end tests for voice → command, voice → agent, cancellation, provider fallback, offline and privacy mode by M8 (§115) <!-- synced:  -->
<!-- /items -->
...
<!-- items:M1 -->
**86 items** · ✅ 0 done · 🟡 1 partial · ⏭ 0 dropped · ⬜ 85 not started

> Generated by `pnpm docs:sync` from the specs. Mark items **in their spec** (with the `→` note), then re-run.

#### [ARCHITECTURE.md](architecture/ARCHITECTURE.md) (9)

- [ ] **ARCH-06** · Autostart registers `kivo-runtime.exe --autostart`; the runtime then launches `KIVO.exe --background`, which preloads the overlay without showing a window (§1) <!-- synced:  -->
- [ ] **ARCH-07** · Quit (tray or Control Center) stops the runtime, closes the UI and releases the mic; closing a window never quits (§1) <!-- synced:  -->
- [ ] **ARCH-09** · `kivo-infer` runs as a supervised worker (same binary, subcommand), started on demand, restarted on crash; the current turn fails gracefully with a spoken message (§1) <!-- synced:  -->
- [ ] **ARCH-10** · Crash dumps are written locally to `%LOCALAPPDATA%\KIVO\crashes\` and reported on the next start; never uploaded without consent (§1, §5) <!-- synced:  -->
- [ ] **ARCH-20** · A separate high-rate `levels` notification (30–60 Hz) carries audio levels, only while the overlay is animating (§3) <!-- synced:  -->
- [ ] **ARCH-21** · The same protocol runs between the runtime and `kivo-infer`, with methods for streaming audio frames and results (§3) <!-- synced:  -->
- [ ] **ARCH-23** · A subscriber persists the Activity subset to SQLite (§4.1) <!-- synced:  -->
- [ ] **ARCH-26** · Stop, Esc, the overlay X, the emergency stop and barge-in cancel the turn token, and cancellation reaches every layer within 100 ms, enforced by a test (§4.2) <!-- synced:  -->
- [ ] **ARCH-28** · `tracing` spans T0 wake … T10 completion carry `turn_id`; a metrics subscriber stores per-turn timings in `turn_metrics` (§4.3) <!-- synced:  -->

#### [BENCHMARKS.md](architecture/BENCHMARKS.md) (2)

- [ ] **BENCH-08** · `e2e` suite: T0–T10 spans for the three plan §102 journeys with scripted audio via a virtual mic (§1) <!-- synced:  -->
- [ ] **BENCH-12** · Budgets met: runtime idle RAM ≤ 150 MB, UI idle RAM ≤ 120 MB (overlay preloaded, CC closed), runtime cold start ≤ 1.5 s, Control Center open (warm) ≤ 300 ms (§3) <!-- synced:  -->

#### [BRAINS.md](architecture/BRAINS.md) (3)

- [ ] **BRAIN-01** · Grammar stage: slot-based commands (`open {app}`, volume, `mute`, media, screenshot, window commands, `lock`) resolved against live app and window indexes, with phrasing loaded from per-language data files (§2) <!-- synced:  -->
- [ ] **BRAIN-02** · Destructive commands (shutdown, restart) still go through the permission engine (§2) <!-- synced:  -->
- [ ] **BRAIN-05** · Metrics `fast_path_ratio` and per-stage p95 latency (§2) <!-- synced:  -->

#### [CAPABILITIES.md](architecture/CAPABILITIES.md) (3)

- [ ] **CAP-01** · Capability model in the runtime: every §1 capability with its default; a disabled capability's tools are **not registered** for any brain, routine, agent, MCP exposure or remote client (§1, §2) <!-- synced:  -->
- [ ] **CAP-02** · A request that needs a disabled capability gets a plain answer ("Screen awareness is off. Turn it on?") with a one-tap link; toggles never turn on automatically (§2) <!-- synced:  -->
- [ ] **CAP-03** · Every toggle change is written to the audit log (§2) <!-- synced:  -->

#### [DISCOVERY.md](architecture/DISCOVERY.md) (2)

- [ ] **DISC-13** · Live state (Island, Tasks, Activity, agent progress, usage) is pushed over IPC; nothing polls it (§3) <!-- synced:  -->
- [ ] **DISC-19** · Zero background work while the Control Center is closed beyond what the runtime needs (§3) <!-- synced:  -->

#### [DISTRIBUTION.md](architecture/DISTRIBUTION.md) (6)

- [ ] **DIST-01** · NSIS per-user installer (no admin, `%LOCALAPPDATA%\Programs\KIVO`) built from the Tauri bundle, with `kivo-runtime.exe` and `kivo-infer.exe` as `externalBin` sidecars (§1) <!-- synced:  -->
- [ ] **DIST-02** · Bundle contents: WebView2 bootstrapper (Windows 10), sounds, icons, default intent grammar, "Hey Kivo" model (from M2), Silero VAD; no STT, TTS or LLM models (§1) <!-- synced:  -->
- [ ] **DIST-05** · Installer registers the startup option, Start-menu shortcut and uninstaller; supports in-place upgrades (§1, plan §110) <!-- synced:  -->
- [ ] **DIST-12** · Model manager (`kivo-store::models`): per-model manifest, resumable HTTP-range downloads, sha256 verification, atomic install into `%LOCALAPPDATA%\KIVO\models`, sources Hugging Face or KIVO's release mirror (§4) <!-- synced:  -->
- [ ] **DIST-13** · Licenses and attribution (CC-BY, OpenRAIL) shown before download and in About; Voice → Models on this PC shows disk usage and Remove (§4) <!-- synced:  -->
- [ ] **DIST-15** · No telemetry by default; crash dumps stay local; the diagnostics bundle is generated on request and reviewed before sharing (§5) <!-- synced:  -->

#### [RELEASE.md](architecture/RELEASE.md) (2)

- [ ] **REL-05** · `release.yml` on `v*` tags, Windows x64 first: sidecars built and copied to `src-tauri/binaries/<name>-<target-triple>`, `tauri-action` → draft GitHub Release with NSIS `.exe`, `.msi` and `latest.json` (§2) <!-- synced:  -->
- [ ] **REL-06** · Install/launch smoke test on a clean Windows runner (silent install, start the runtime, IPC health, uninstall) and checksums before publishing a draft (§2) <!-- synced:  -->

#### [SECURITY.md](architecture/SECURITY.md) (8)

- [ ] **SEC-01** · Permission modes Ask every time and Auto (default) in the engine (§1.1) <!-- synced:  -->
- [ ] **SEC-04** · Switching mode only through UI or hotkey (Ctrl+Shift+M), from the Island, tray, chat and Home; never through a tool call or voice alone; the Island shows the mode while acting (hidden for Auto) (§1.1) <!-- synced:  -->
- [ ] **SEC-05** · Hard limits enforced in every mode: emergency stop, disabled capabilities, blocked apps, no typing into password fields, destination binding, per-task cost caps (§1.1) <!-- synced:  -->
- [ ] **SEC-06** · `authorize(ToolCall, Context) -> Decision { Allow | Confirm(ConfirmSpec) | Deny(reason) }` with the §2 `Context`, in `kivo-security` (§2) <!-- synced:  -->
- [ ] **SEC-07** · Default policy table (risk × clean/tainted/guest) implemented and unit-tested; High always confirms except in Bypass (§2) <!-- synced:  -->
- [ ] **SEC-10** · Confirmation card: exact action in plain words, target, why, provenance; Allow once / Always for… / Deny; never auto-dismisses (§2) <!-- synced:  -->
- [ ] **SEC-22** · Append-only `audit` table with the §7 fields and `hash = sha256(prev_hash || row)` (§7) <!-- synced:  -->
- [ ] **SEC-25** · Emergency stop from the Ctrl+Alt+Shift+Esc hotkey (low-level hook fallback), tray "Stop everything" and the overlay/Control Center Stop button (§8) <!-- synced:  -->

#### [TOOLS_AND_CONTROL.md](architecture/TOOLS_AND_CONTROL.md) (11)

- [ ] **TOOL-01** · `ToolSpec` and `ToolResult` exactly as in §1 (namespaced id, JSON Schema params/result, risk, side effects, data egress, timeout, cancellable, capability tier, platforms), plus `undo` handler or `irreversible: true` (§1, UX §8.1) <!-- synced:  -->
- [ ] **TOOL-02** · The executor API requires the `Decision` from `authorize()`; nothing executes without one (type-level) (§1, SECURITY §2) <!-- synced:  -->
- [ ] **TOOL-03** · Errors carry a user-readable message and a machine code; raw HRESULTs are logged, never spoken (§1) <!-- synced:  -->
- [ ] **TOOL-06** · Apps: launch, close, focus, restart, list installed (Start-menu `.lnk` + `shell:AppsFolder`, UWP via AUMID, fuzzy match with aliases) (§3) <!-- synced:  -->
- [ ] **TOOL-07** · Windows: list, focus (foreground-lock rules), minimize, maximize, close; DWM cloaking check (§3) <!-- synced:  -->
- [ ] **TOOL-09** · Audio: volume get/set, mute, mic mute (§3) <!-- synced:  -->
- [ ] **TOOL-11** · Media: play/pause/next/previous and now-playing via `GlobalSystemMediaTransportControlsSessionManager` (§3) <!-- synced:  -->
- [ ] **TOOL-12** · System: lock and sleep; restart and shutdown as High risk with confirmation (§3) <!-- synced:  -->
- [ ] **TOOL-14** · Screen: screenshot of screen, window or region (Windows.Graphics.Capture) (§3) <!-- synced:  -->
- [ ] **TOOL-18** · Notifications: show a Windows toast (§3) <!-- synced:  -->
- [ ] **TOOL-23** · Open URL and web search via the default browser (§5) <!-- synced:  -->

#### [UX.md](architecture/UX.md) (18)

- [ ] **UX-01** · First launch opens the Control Center on onboarding; a manual launch starts the runtime and shows the Control Center; a relaunch shows, unminimizes and focuses the main window (§1) <!-- synced:  -->
- [ ] **UX-02** · Close (X, Alt+F4, taskbar) hides the window and KIVO keeps running ("On close, keep KIVO running", on); the first close shows a one-time toast with Settings and Quit (§1) <!-- synced:  -->
- [ ] **UX-03** · Tray: left-click opens the Control Center; right-click menu Open KIVO · Pause/Resume listening · Hide overlay for 1 hour · Stop everything · Settings · Quit KIVO (§1) <!-- synced:  -->
- [ ] **UX-04** · Tray icon states: normal, listening, paused (slashed mic), error (badge), updating (§1) <!-- synced:  -->
- [ ] **UX-56** · Tray tooltip: "KIVO · <state>" plus the permission mode and running-task count (mockup → System surfaces) <!-- synced:  -->
- [ ] **UX-57** · Actionable Windows notifications with buttons routed back to the runtime: first close (Settings / Quit KIVO), microphone blocked (Open Windows settings / Type instead); later sources reuse the same template (§1, §7) <!-- synced:  -->
- [ ] **UX-06** · The Island window: top center of the monitor with the foreground window, ~8 px from the top, never takes focus, draws nothing when hidden; hidden windows set WebView2 `IsVisible=false` (§2, §3) <!-- synced:  -->
- [ ] **UX-07** · Island states driven by the runtime's `SessionState`: listening (live transcript), thinking, acting (target-app icon, steps), speaking, awaiting confirmation, error, paused (§2) <!-- synced:  -->
- [ ] **UX-09** · Card: ≤ 520 px wide and ≤ 50% of screen height, scrolls, focusable only while typing; header brain/profile chip (routing reason on hover, M3); body with the editable transcript (click to fix and resend), answer and step rows; footer text field, mic toggle, Stop and "Open in Control Center" (§2) <!-- synced:  -->
- [ ] **UX-10** · End: collapses 4 s after the answer, not while hovered, typing or confirming; confirmations never auto-dismiss (§2) <!-- synced:  -->
- [ ] **UX-11** · Fullscreen app / Focus mode: suppressed or tiny pill (setting), sounds only (§2) <!-- synced:  -->
- [ ] **UX-12** · Keyboard: Esc cancels, Ctrl+Enter sends, Tab moves between confirmation buttons (§2) <!-- synced:  -->
- [ ] **UX-19** · Home: status orb and "Listening for Hey Kivo", Talk / Pause listening / mode picker, Running and Recent lists (mockup → Home) (§3, plan §81) <!-- synced:  -->
- [ ] **UX-20** · Activity: timeline of turns, tool calls and results from the Activity table (mockup → Activity) (§3, plan §83) <!-- synced:  -->
- [ ] **UX-41** · Ctrl+Shift+Space opens the card in text mode with focus in the field; behaves like a spoken request; no speech reply unless enabled (§8) <!-- synced:  -->
- [ ] **UX-49** · Every UI string goes through i18n (`i18next`, ICU messages), English source locale; no hard-coded strings (§9) — note: the current component gallery has hard-coded English, to be moved to i18n in M1 <!-- synced:  -->
- [ ] **UX-50** · CSS logical properties so RTL works by switching `dir`; dates and numbers via `Intl` (§9) <!-- synced:  -->
- [~] **UX-52** · ARIA roles, full keyboard navigation and visible focus rings in the overlay and Control Center (§10) → partial: components use Base UI (ARIA, keyboard) and a focus ring · missing: overlay window, audit <!-- synced:~ -->

#### [VOICE.md](architecture/VOICE.md) (14)

- [ ] **VOICE-02** · Resample to 16 kHz mono (`rubato`); 10 ms internal frames batched to 80 ms for models (§1) <!-- synced:  -->
- [ ] **VOICE-03** · Capture on an MMCSS "Audio" thread writing a lock-free ring buffer of ≥ 3 s; detection on one worker thread with EcoQoS while idle (§1) <!-- synced:  -->
- [ ] **VOICE-04** · Energy gate → Silero v6 VAD (`ort`) (§1, §3) <!-- synced:  -->
- [ ] **VOICE-06** · Traits `VadEngine`, `WakeDetector`, `WakeVerifier`, `SpeakerVerifier`, `SttEngine` (partial/stable/final events), `TtsEngine` (streaming) and `TurnDetector`; every engine declares `EngineInfo` (id, kind, license, languages, streaming, accel, resource estimate) (§2) <!-- synced:  -->
- [ ] **VOICE-07** · Cloud engines pass the privacy check before any audio leaves the device (§2, SECURITY §6) <!-- synced:  -->
- [ ] **VOICE-08** · Default streaming STT engine (the M0 winner among Moonshine v2, Parakeet TDT v3 and Whisper-turbo) running in `kivo-infer`, with partial transcripts shown live (§3) <!-- synced:  -->
- [ ] **VOICE-09** · TTS: system voices (WinRT SpeechSynthesizer / SAPI 5) and Kokoro-82M (EN via misaki, no espeak), streaming from `kivo-infer` (§3) <!-- synced:  -->
- [ ] **VOICE-41** · Push-to-talk: hold Ctrl+Space to talk (`Hotkeys` trait: RegisterHotKey with a low-level-hook fallback), optional toggle mode, auto-end on silence; registration conflicts (e.g. IME switching on CJK layouts) detected with a rebind prompt (DECISIONS "Activation", UX §5) <!-- synced:  -->
- [ ] **VOICE-24** · Earcons `listen_start`, `listen_stop`, `error`, `done`, `thinking` (off; after 1 s) and `hangup`, each < 300 ms, pre-decoded into the playback mixer; wake → audio ≤ 150 ms; Kenney CC0 placeholders (§6) <!-- synced:  -->
- [ ] **VOICE-25** · Capture continues during earcons; AEC removes them and VAD/STT input is gated for the earcon plus 50 ms (§6) <!-- synced:  -->
- [ ] **VOICE-34** · Residency: capture, VAD, wake and spotter always resident while listening; STT and TTS warm for 10 min after use (setting); STT prewarm on the wake-stage-1 hit and TTS prewarm on turn start (§8) <!-- synced:  -->
- [ ] **VOICE-36** · `LanguagePack { code, stt_engines, tts_voices, kws_model?, grammar, vocabulary, status }` with English first; the router picks the engine for the active language (§9) <!-- synced:  -->
- [ ] **VOICE-37** · Primary + secondary language settings; automatic language ID only where the engine provides it and it measures reliable (§9) <!-- synced:  -->
- [ ] **VOICE-40** · Every §10 budget is measured by `kivo-bench` on the reference tiers and met, or the miss is logged in DECISIONS.md (§10) <!-- synced:  -->

#### [KIVO_Project_Plan.md](KIVO_Project_Plan.md) (8)

- [ ] **PLAN-01** · Hardware detection through `SystemInfo`: CPU, RAM, GPU, VRAM, NPU where supported, battery state and current load; feeds the voice recommendation (§25, §90) <!-- synced:  -->
- [ ] **PLAN-02** · Model residency states Unloaded → Warming → Warm → Active → Idle → Unloading, tracked per model and reported over IPC (§26, §93) <!-- synced:  -->
- [ ] **PLAN-11** · Fast acknowledgement: long operations show immediate visual feedback in the Island, with no spoken filler (§36, §95) <!-- synced:  -->
- [ ] **PLAN-14** · Startup sequence: runtime → lightweight config → event bus → audio → wake/VAD → OS event registration, with heavy models loaded only by policy (§126) <!-- synced:  -->
- [ ] **PLAN-15** · Shutdown sequence: stop new tasks, notify and cancel active work, stop audio and providers, persist state, close the database, exit workers, then the runtime (§127) <!-- synced:  -->
- [ ] **PLAN-16** · The ten resource rules are reviewed at each milestone exit, with any exception logged in DECISIONS.md (§128) <!-- synced:  -->
- [ ] **PLAN-18** · Error UX: plain-language cause plus Retry / Open settings actions, never raw codes (§146) <!-- synced:  -->
- [ ] **PLAN-19** · Acceptance journey §138 "Kivo, mute": no external AI request, brief confirmation <!-- synced:  -->
<!-- /items -->
...
- [ ] **UX-33** · Steps 1–5: Welcome (black, Island demo), Microphone check, How you call KIVO, Hearing & speaking (engine + voice, background download), Your voice (optional enrollment with consent); steps slide 18 px in the direction of travel and the progress dots stretch (DESIGN_SYSTEM §5) (§4) <!-- synced:  -->
...
- [ ] **UX-21** · Chat: threads list, conversation with brain switcher, attachments, tool activity, cancel, context meter, Compact now (mockup → Chat; CONVERSATION §0–1) <!-- synced:  -->
...
- [ ] **UX-24** · Tasks: running and past tasks with status, current step, elapsed time, tool activity, cancel, result (mockup → Tasks; plan §84) <!-- synced:  -->
...
- [ ] **UX-31** · Settings tabs General · Appearance · Island · Sounds · Notifications · Accessibility · Shortcuts · Performance · Diagnostics · About, with the §5 defaults (mockup → Settings) <!-- synced:  -->
...
- [ ] **REL-08** · `nightly.yml` for the Experimental channel (artifacts expire after 14 days) (§2) <!-- synced:  -->
