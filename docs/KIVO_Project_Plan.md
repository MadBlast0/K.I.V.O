...
- [x] **PLAN-09** · M8 · GPU policy: consider GPU load, VRAM, active apps, battery, gaming and model needs; never compete with GPU-heavy work (§91) → done: `gpu.rs` puts the recognizer on the GPU (whisper.cpp on Vulkan; DirectML until 2026-09-25) only when the engine can use it, the profile allows it, no fullscreen app is in front, the card has ≥ 3 GB VRAM and its load is under 60 %; it leaves above 60 % and comes back under 30 % (hysteresis); a crash on the GPU moves it to the CPU for the session (PLAN-12) · verified: `gpu::tests`, `models::the_recognizer_leaves_the_gpu_for_games_and_busy_gpus`, Parakeet on the RTX 3060 (229 ms vs 398 ms on CPU) (2026-09-24); Parakeet is CPU-only since DirectML was removed (2026-09-25)
