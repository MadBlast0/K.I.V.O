...
- [x] **PLAN-20** · M5 · Acceptance journey §137 (coding project: open, find failing test, fix, validate, summarize, speak) → done: "fix the failing test" runs the Coding agent in the project, shows its steps, re-runs the tests itself, has the agent retry once, summarizes and says it's fixed only when they pass · verified: `a_coding_request_is_fixed_checked_and_reported` (with a scripted ACP agent; a live Claude Code run needs its adapter, M3-open) (2026-09-23)
- [x] **PLAN-21** · M7 · Acceptance journey §139 (confidential document summarized with a local model, cloud blocked) → done: "Summarize this confidential document locally" is seen as a privacy requirement (`privacy::asks_to_stay_local`, the language's word list), the cloud is blocked for it, the local brain reads the file on this PC and answers · verified: `m7_acceptance::a_confidential_document_is_summarized_on_this_pc`, `kivo-security` `a_request_can_ask_to_stay_on_this_pc` (2026-09-24)
- [x] **PLAN-22** · M5 · Acceptance journey §140 (build watcher: no LLM while waiting, notify on completion) → done: "tell me when the build finishes" finds the build, waits on the process with no brain call, and tells the user · verified: `the_build_watcher_waits_without_ai_and_tells_you` (2026-09-23)
- [x] **PLAN-23** · M8 · Acceptance journey §141 (where is the Export button: UIA bounds, companion points, voice explains) → done: "where is the Export button" → UIA bounds → the Island points beside it → "It's at the top right of Test App" (UX-39) · verified: `kivo_points_at_a_control_and_says_where_it_is`, `uia_testenv` Export bounds
...
- [x] **PLAN-27** · M7 · MVP definition (§156) fully met → internal alpha → done: every §156 item exists and is tested: Windows runtime, wake word, push-to-talk, Moonshine STT, Kokoro/Windows TTS, cloud brains, a local brain, CLI agents over ACP, native commands, app launch, files, browser launch, UI Automation, permissions, Control Center, logging, cancellation, the Island companion; internal alpha is the owner's first `v*` tag · verified: the M1–M7 journeys (`spoken_command`, `brain_turns`, `control_turns`, `m5_journeys`, `m6_extensions`, `m7_*`) (2026-09-24)
- [~] **PLAN-28** · M8 · Beta definition (§157) fully met → partial: of §157, built and verified — multiple cloud providers, provider profiles, CLI discovery, dependency management (DIST-14), several STT/TTS engines, browser automation, MCP, background tasks and their history, privacy modes, the permission system, provider fallback, and the companion (Island, Orb, Character) · missing: the benchmark runs (the `kivo-bench` harness is built; runs deferred by the owner)
- [~] **PLAN-29** · M9 · Production definition (§158) fully met → partial: of §158, built and tested — the runtime's supervision and recovery, the security suite, the updater with rollback, diagnostics, broad provider support, computer control on the dummy app, performance profiles, accessibility, the specs and automated tests, and safe defaults · missing: evidence only use and releases give: a signed release updating itself in the field, the benchmark runs, a week of daily use
- [~] **PLAN-30** · M9 · Quality gates (§159) pass for the release → partial: the gates that tests can show are green — no known permission bypass (security suite), provider failures don't crash KIVO, core Windows actions deterministic on the dummy app, interruption and cancellation · missing: no known critical crash after daily use, the idle budget and wake reliability measured (bench runs deferred), onboarding reviewed by the owner
- [x] **PLAN-31** · M9 · Architecture invariants checklist (§160, ARCHITECTURE §8) all green → done: `pnpm invariants` (`scripts/invariants.mjs`) runs the tests that enforce each of the twelve invariants (ARCHITECTURE §8) and counts a skipped test as not proven; new checks for invariant 2 (`the_app_reaches_kivo_only_through_ipc`) and 10 (`Secret` has no Display, Serialize or Clone: compile-fail doc tests) · verified: all twelve green on this PC with the speech-model tests run (2026-09-24)
