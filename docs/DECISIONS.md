...
| **Extensions page (redesign)** | One page, no tabs: **Installed / Browse** switch plus **All · Apps · Tools · Skills** filters (plain names for connectors, MCP servers and skills). Installed is grouped into Built in, Apps, Tools and Skills, each with an on/off switch; Browse is a card catalog. One **Add** button (app, tool link or skill). Every item opens a detail panel listing what KIVO can do, with Auto/Ask/Off per action |
