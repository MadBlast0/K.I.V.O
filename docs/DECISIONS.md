...
| Topic | Decision |
|---|---|
| **Supertonic 3 is the Instant tier** | VOICE-11 named Supertonic-2; Supertonic 3 (M2, same publisher, more languages) fills the Instant tier instead. Chatterbox Turbo's reference voice is a Windows voice, rendered locally, so no voice sample is bundled |
| **GPU through DirectML** | *Amended by "Local models on the GPU first" (2026-09-24): Whisper ONNX crashes DirectML and stays on the CPU; the GPU engines are whisper.cpp on Vulkan, and Parakeet keeps only its encoder on DirectML.* Parakeet runs on DirectML with CPU fallback; the GPU policy (PLAN-09) moves the recognizer off the GPU above 60 % load, back under 30 %, never with a fullscreen app in front or on Gaming. A crash on the GPU keeps it on the CPU for the session |
| **Cloud speech keys** | Cloud STT/TTS keys live in Credential Manager, are tested before saving and never enter model context; nothing here has been run against the live services (no owner keys on this PC) |
| **Product modes restore exactly** | A mode (Private, Offline, Battery, Performance, Gaming, Presentation) changes only its part; what Normal looked like is kept in the database, so Normal brings back the user's own settings even after a restart |
| **Hybrid memory search** | sqlite-vec 0.1.9 (`vec0`, 384-d cosine) holds one MiniLM vector per note, with the model per vector in a profile-scoped table; FTS5 and vector hits are fused by reciprocal rank; vector hits beyond cosine distance 0.65 are dropped. The MiniLM model is the intent router's, loaded once and shared |
| **Near-duplicates need the same specifics** | Two notes are the same fact only when their names, numbers and paths match and their MiniLM cosine is ≥ 0.9 (measured: a paraphrase 0.93; "10 am" vs "11 am" 0.91 but different specifics, so kept apart) |
| **Event triggers are sampled** | Routine event triggers (app launched, USB device, Wi-Fi, idle/return, battery) are sampled every 5 s rather than hooked; schedules use cron with a time zone (jiff, DST-safe). A triggered routine is unattended: its High-risk steps always ask on screen |
| **Signed catalogs on by default** | The connector, plugin, model and CLI-install catalogs are fetched daily (off in Strictly private or with "Check for catalog updates" off) and used only with a valid ed25519 signature from `catalogs/keys.txt`, which is empty until the owner adds the signing key |
| **Desktop AI apps' hints start unverified** | The Claude, ChatGPT and Copilot registry entries carry `[ai]` UI Automation hints marked `verified = false`; when the controls aren't found KIVO falls back to the clipboard and "press Enter when ready" |
| **OAuth client ids at build time** | KIVO's own Google and Microsoft OAuth clients are public PKCE clients; their ids come from `KIVO_GOOGLE_CLIENT_ID` / `KIVO_MICROSOFT_CLIENT_ID` at build time. Without them the native connectors say they aren't set up |
| **Mistral's streaming dialect** | Mistral's OpenAI-compatible API rejects `stream_options`, so usage streaming is off for it (`stream_usage = false`) |
| **Computer use is asked for by name** | `computer.use` is offered to a brain only when the request asks for it in so many words ("use the computer", "take control"), as the last rung; watch mode covers the first 10 tasks. Pause-on-mouse counts input only when it comes at least a second after KIVO's own last action (idle time is in whole seconds, and KIVO's input resets it too) |
| **Realtime voice** | OpenAI Realtime (`gpt-realtime`, voice "marin") and Gemini Live (`gemini-2.5-flash-native-audio-preview-09-2025`, voice "Puck") over `tokio-tungstenite`; the Gemini key goes in the `x-goog-api-key` header, never the URL. A session opens only after the fast path, for a conversation-type request, a profile with "Realtime conversation" on, or the card's "Talk live". The wake-word listener's own utterances are ignored during a session ("Kivo stop" still works); a declined action declines only that action. A new session input, `LiveListening`, takes the session back to Listening between turns |
| **Flaky under load** | `spoken_command::talking_over_kivo_works_with_speakers_echoing_its_voice` (Moonshine under simulated echo) and `voice::tests::silence_after_speech_ends_the_utterance` can fail while the full batch loads the machine; both pass alone |

## 2026-09-24 — M9 build

| Topic | Decision |
|---|---|
| **The runtime runs updates** | Not the webview's `tauri-plugin-updater`: the runtime holds the exe locks, winds turns down and must survive the app's restart, and the overlay/Control Center never get to run an installer. It keeps the plugin's formats (minisign `.sig`, `latest.json` manifests, `/P /R /UPDATE` for NSIS and `msiexec /passive AUTOLAUNCHAPP=True` for MSI), verified with `minisign-verify`, the crate the plugin uses |
| **Channels on one `updates` release** | Each channel's manifest is an asset of a fixed `updates` release (`stable.json`, `beta.json`, `experimental.json`), overwritten when a release is published. The installed build's channel is the default; the user can choose another in Settings → About |
| **Both signatures, when there are both** | An installer runs only after its minisign signature checks out against the key compiled into the runtime, and, in a code-signed build, its Authenticode signature (whole chain, revocation checked) names KIVO's publisher (`KIVO_SIGNING_PUBLISHER`). A build made before the owner has a code-signing certificate checks the minisign signature only |
| **Rollback** | The running version's installer is kept (or fetched and verified from its release) before an update. The runtime counts its starts and marks the update healthy after 15 s; the app retries a runtime that doesn't come up and, after two failed starts, runs the kept installer. Only the current and previous installers are kept |
| **Every publish is approved** | `release.yml` builds, checks and drafts; the `publish` job waits for the `production` environment's reviewers for every channel, then publishes the release and its channel manifest. Experimental releases are removed after 14 days |
| **Notices without cargo-about** | `THIRD_PARTY_NOTICES.txt` comes from `pnpm licenses:gen`, which already walks the shipped crates and npm packages: it copies each package's own licence and notice files (identical texts once, 1.3 MB). CycloneDX SBOMs come from `cargo cyclonedx` and `cdxgen` in the release workflow |
| **Signing waits for the owner** | `scripts/sign.ps1` signs through Azure Artifact Signing or an OV certificate, whichever the owner sets up; until then builds are unsigned and the workflow says so |
| **Character drawn in SVG** | UX-38's Character is KIVO's own SVG mascot (`Character.tsx`) with the spec's expressions, instead of a Rive asset, which doesn't exist yet. Its moods come from the same `SessionState`; nothing animates at rest. A designed Rive character can replace the drawing later without changing the states or the setting |
| **Recall of old messages by meaning** | CONV-06: every kept message gets a MiniLM vector in sqlite-vec (`message_vectors`, migration 12, profile-scoped), embedded in the background with the notes; a thread's summarized messages are recalled by words and meaning, fused by rank, with the notes' 0.65 cosine-distance cut-off |
| **A request that ends before the worker starts** | A voice request that ended on silence before the speech worker had started it lost its audio ("I didn't catch that" on a cold or slow start). The listener now keeps that audio until the worker is ready, and the engine waits until it is sent before asking for the transcript |
| **CLI brains over ACP, not a PTY** | Owner asked which is better (2026-09-24). CLI agents run as hidden child processes speaking ACP (JSON-RPC over stdio): structured text, plans, tool calls and file edits, and permission requests that go through KIVO's permission engine. A PTY would give only a screen to scrape, with no reliable end of an answer and no permission hook, and breaks when a CLI changes its layout. Watching an agent work stays the visible Windows Terminal launch (DECISIONS "Terminal agents"); no hidden PTY is added |
| **Engine benchmark in the app** | BENCH-15: "Benchmark this engine" measures local engines only, on commands a Windows voice speaks (clean and with noise), in a separate test worker. Its thresholds are KIVO's (`kivo_voice::registry::thresholds`): STT final ≤ 300 ms (VOICE §10), TTS first audio ≤ 300 ms, RTF ≤ 1, cancel ≤ 100 ms, WER ≤ 10% clean and ≤ 20% with noise |
| **Agent setup time limit** | Starting a CLI agent and opening its session time out after 60 s ("it may need you to sign in"); stopping an agent ends its whole process tree. Found by the live Claude Code/Codex run: one silent agent held every agent request, and npm shims left the agents running |
| **Long speech in segments** | Found by the `stt` benchmark (2026-09-24): Moonshine's model fails on audio past about 9.5 s ("the voice engine stopped working" on any long request), and Whisper heard only the first 30 s. The shared utterance helper now cuts speech longer than an engine takes (`Transcriber::max_samples`: Moonshine 9 s, Whisper 29 s) at the quietest 20 ms in each segment's last 40%, keeps finished segments' text, and re-transcribes only the growing segment for partials |
| **`stt` suite on KIVO's engines** | The `stt` suite measures KIVO's own engines in the `kivo-infer` worker, cold-loaded per run, instead of sherpa-onnx's versions of the same models: the numbers are what KIVO ships, and they feed the Voice page's registry (Moonshine, Parakeet, Whisper) |
| **Local models on the GPU first** | Owner (2026-09-24): local AI models run on the GPU by default, whatever one PC measures, with a user switch (Settings → Performance → "Speech recognition on the graphics card", on by default) and the processor as the fallback when there is no usable card, a game or full-screen app is in front, the Gaming/Battery profile is on, the PC is on battery or the GPU is over 60% busy. The GPU runtime is whisper.cpp (GGML models, Vulkan), the way desktop dictation apps run Whisper: three downloadable models from the whisper.cpp project at a pinned revision (Whisper small q8, large-v3-turbo q5, base.en), first in the Recommended, Multilingual, High-accuracy and Lightweight cards when the switch is on. Measured on the owner's PC: KIVO's int8 ONNX models do badly on DirectML (Parakeet streaming 0.95× real time on the RTX 3060 vs 0.57× on the CPU even with only its encoder there; Whisper's and Moonshine's files crash DirectML; Kokoro's ConvTranspose fails there), so ONNX engines other than Parakeet stay on the processor. The always-on detectors (VAD, wake word, speaker check) stay on the processor so the card isn't kept awake. Only Windows x64 builds get whisper.cpp (`kivo_voice::whisper_cpp::AVAILABLE`); ARM64 and the unpublished macOS/Linux builds don't offer its engines. Building the worker needs CMake plus the Vulkan SDK's headers, `glslc` and loader library, and libclang for whisper-rs's bindings (the ones it bundles are Linux's and fail on Windows): `pnpm build-tools` fetches them, pinned and checked, into `.tools/` without an installer or admin rights (the LunarG installer needs admin, which the owner's PC declined), and `pnpm dev`/`pnpm build` run it first; CI installs LunarG's SDK (pinned, SHA-256) and uses the runner's LLVM. ggml's nested shader-generator build exceeds Windows' 260-character paths inside Cargo's OUT_DIR, so `crates/kivo-voice/whisper-cpp-paths.cmake` (through `CMAKE_PROJECT_INCLUDE`) moves it to a short folder under `target/ep`. Installed KIVO needs only the graphics driver's Vulkan loader |
| **Kokoro in fp16** | KIVO downloads Kokoro's `model_fp16.onnx` (163 MB) instead of `model_quantized.onnx` (92 MB): on the owner's CPU it starts speaking in 0.5 s instead of 1.5 s and runs at 0.28 instead of 0.87 × real time (fp32 is as fast at twice the size). Pinned at the same revision; SHA-256 checked against Hugging Face's record. Older downloads keep working |
| **Speech workers exit when stopped** | Found by the `stt` benchmark: a stopping `kivo-infer` hung in tokio's runtime shutdown (its blocking stdin reader) and its answer to `shutdown` could be lost, while the runtime waited for that answer with no time limit before killing it. Every cool-down, "Try sample", voice preview, engine switch and benchmark left a ~37 MB worker behind. The worker now ends with a short `shutdown_timeout`, and the runtime waits at most 2 s before killing it |
| **Wake sensitivity 0.8** | `kivo-bench wake` on KIVO's own spotter (2026-09-24), 66 synthetic "Hey Kivo"s and 5.4 h of LibriSpeech: at 0.5, 12% missed clean and 52% over a talker at −10 dB; at 0.8, 9% and 45%; no false accept in 5.4 h at either. "Hey Kivo" starts at 0.8, and a built-in word still on 0.5 moves (migration 13). Hearing it over other people talking stays the open-vocabulary spotter's weak spot and misses VOICE §10's 5% false-reject budget; KIVO trains no models (VOICE-13 dropped), so the remedy is a better published keyword model when one exists, not more sensitivity |
| **Wake suite on KIVO's spotter** | The `wake` suite measures KIVO's own keyword spotter with the runtime's "Hey Kivo" keywords instead of sherpa-onnx's, and splits the negatives into one slice per run so the corpus is heard once (p50/p95 across slices) |
| **Audio in pieces under the frame limit** | Found by the `tts` benchmark: Supertonic returns a long reply in one piece, and 20 s of 44.1 kHz audio is about 2.4 MB, over the IPC's 1 MiB frame limit (SECURITY §9), so the runtime dropped the worker. The worker now sends at most a second of audio a message, whatever size an engine produces |
| **Interruptible model runs** | Found by the `tts` benchmark: after a cancel, Kokoro finished the sentence it was computing (2.9 s on a long one) and the next reply waited for it. A run now gets ONNX Runtime `RunOptions` that a small watcher terminates as soon as the request is cancelled (`kivo_voice::interrupt`) |
| **Turn ids unique across runs** | Turn ids counted from `t1` at every start, so after a restart a new request collided with a stored one: it wasn't recorded in Activity or the audit (`UNIQUE constraint failed: turns.id`) and its rows joined an old entry. The counter now starts at the runtime's start time in milliseconds |
| **Accented English from EdAcc** | BENCH-02's accented WER uses EdAcc (University of Edinburgh, CC BY-SA 4.0), the accented set of the Open ASR Leaderboard, instead of L2-ARCTIC (sign-up form) or Common Voice (account needed): one test shard from the publisher's Hugging Face repository at a pinned revision, 40 turns of 2–20 s taken in turn from every accent in it, annotations removed from the references. It is spontaneous conversation, so its WER is higher than LibriSpeech's for every engine; it compares engines, it isn't a budget |
| **Speech timed live** | VOICE §10's "end of speech → final ≤ 300 ms" is about a request KIVO heard as it was spoken, while it was already transcribing. The `stt` suite now feeds five utterances in real time, 80 ms at a time, and times the first partial and the end of speech → final; giving the engine a whole recording at once (the old "end → final") is kept as "all audio at once", the cost of a long file |
| **Wake word timed** | The `wake` suite measures how soon after "Hey Kivo" ends the spotter reports it (clean clips in 20 ms pieces, plus its compute time), the larger part of VOICE §10's wake → earcon; the cue itself starts about 6 ms later (e2e). The negatives are LibriSpeech test-clean and test-other, 10.7 h |
| **GGML on Vulkan, no DirectML** | Owner (2026-09-25): local speech models run like Handy's: GGML engines on the graphics card through Vulkan, the CPU as fallback. DirectML is not used for local models (it failed or ran slower for Kokoro, Moonshine, Whisper ONNX and Parakeet here). Speech-to-text already has whisper.cpp on Vulkan; text-to-speech (Kokoro, Supertonic on ONNX, CPU today) moves to a GGML voice engine on Vulkan, and Parakeet's DirectML encoder goes. Next session: VOICE-50. Also open: Supertonic's cancel takes 3.6 s (Kokoro's interruptible runs weren't applied to it) |
| **GPU backend per machine** | Owner (2026-09-25): NVIDIA → CUDA by default (Vulkan selectable), AMD/Intel → Vulkan, Mac → Metal (Vulkan through MoltenVK selectable), no usable GPU → CPU; Settings → Performance → Graphics backend (Automatic / CUDA / Vulkan / Metal / Processor only, only what the machine has). The CUDA runtime is an on-demand download, not in the installer. Plan and repositories (whisper.cpp, Handy, transcribe-rs, TTS.cpp, llama.cpp): `docs/research/gpu-speech/REPORT.md`; built under VOICE-50 |
