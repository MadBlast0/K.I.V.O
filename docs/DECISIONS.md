...
| **GPU through DirectML** | Parakeet (and Whisper when it can) run on DirectML with CPU fallback; the GPU policy (PLAN-09) moves the recognizer off the GPU above 60 % load, back under 30 %, never with a fullscreen app in front or on Gaming. A crash on the GPU keeps it on the CPU for the session |
