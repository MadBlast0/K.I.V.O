    ...
    <>
      {items.map((e, i) => {
        ...
        return (
          <BMenu.Item
            key={i}
            className={cn("k-menu__item", e.danger && "k-menu__item--danger")}
            disabled={e.disabled}
            onClick={e.onSelect}
          >
            {e.checked !== undefined ? (
              <span className="k-menu__check">{e.checked && <Icon name="check" />}</span>
            ) : (
              e.icon && <Icon name={e.icon} />
            )}
            {e.label}
    ...
    <BContextMenu.Root>
      ...
      <BContextMenu.Portal>
        <BContextMenu.Positioner>
          <BContextMenu.Popup className="k-popup">
            <Entries items={items} />
          </BContextMenu.Popup>
        </BContextMenu.Positioner>
      </BContextMenu.Portal>
