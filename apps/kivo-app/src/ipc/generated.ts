...
export type Event = { meta: EventMeta } & (
  | { group: "voice"; event: VoiceEvent }
  | { group: "turn"; event: TurnEvent }
  | { group: "tool"; event: ToolEvent }
  | { group: "task"; event: TaskEvent }
  | { group: "system"; event: SystemEvent }
  | { group: "provider"; event: ProviderEvent }
  | { group: "ui"; event: UiEvent }
);
...
export type VoiceEvent =
  | { type: "wakeDetected"; wordId: string; score: number }
  | { type: "speechStarted" }
  | { type: "partialTranscript"; text: string; stableLen: number }
  | { type: "finalTranscript"; text: string; confidence: number | null }
  | { type: "speechEnded" }
  | { type: "ttsStarted" }
  | { type: "ttsStopped"; reason: TtsStopReason }
  | { type: "bargeIn" };

export type TtsStopReason = "finished" | "cancelled" | "bargeIn";

export type TurnEvent =
  | { type: "started"; source: TurnSource }
  | { type: "intentDetected"; path: IntentPath; intent: string }
  | { type: "brainSelected"; profile: string; provider: string; model: string; reason: string }
  | { type: "completed" }
...
export type CancelReason =
  "userVoice" | "userButton" | "hotkey" | "emergencyStop" | "bargeIn" | "timeout" | "providerFailure" | "shutdown";

export type ToolEvent =
  | { type: "requested"; callId: string; toolId: string }
  | { type: "permissionDecided"; callId: string; decision: PermissionDecision }
  | { type: "started"; callId: string }
  | { type: "progress"; callId: string; message: string }
  | { type: "completed"; callId: string }
  | { type: "failed"; callId: string; message: string };

export type PermissionDecision = "allow" | "confirm" | "deny";

export type TaskEvent =
  | { type: "created"; name: string }
  | { type: "stepChanged"; step: string; status: StepStatus }
...
export type SystemEvent =
  | { type: "windowChanged"; app: string; title: string }
  | { type: "fullscreenChanged"; fullscreen: boolean }
  | { type: "focusModeChanged"; on: boolean }
  | { type: "powerChanged"; onBattery: boolean; percent: number | null }
  | { type: "deviceChanged"; kind: DeviceKind; name: string }
  | { type: "networkChanged"; online: boolean }
  | { type: "fileChanged"; path: string }
  | { type: "browserChanged"; url: string; title: string }
  | { type: "automation"; kind: string; element: string; name: string }
  | { type: "shuttingDown" }
  | {
      type: "modelChanged";
      id: string;
  ...
  | { type: "modelResidency"; engine: string; state: string }
  | { type: "speechFallback"; slot: string; from: string; to: string | null; message: string }
  | { type: "engineSwitch"; slot: string; engine: string; stage: string; message: string | null }
  | { type: "discoveryChanged"; section: string }
  | { type: "configChanged" };

export type DeviceKind = "microphone" | "speaker";

export type ProviderEvent =
  | { type: "healthChanged"; provider: string; healthy: boolean }
  | { type: "rateLimited"; provider: string; retryAfterMs: number | null }
  | { type: "authFailed"; provider: string }
  | { type: "discoveryChanged"; section: string }
  | { type: "usageRecorded"; provider: string; cost: number | null }
  | { type: "threadChanged"; thread: string };

export type UiEvent =
  | { type: "controlCenterRequested"; page: string | null }
  | { type: "overlayShown" }
  | { type: "overlayHidden" }
  | { type: "userConfirmed"; callId: string }
  | { type: "userCancelled" };
...
export type SpeechEngineItem = {
   ...
   */
  fitsLanguage: boolean;
  voices: Array<VoiceItem>;
  /**
...
export type SpeechChoices = {
   ...
   */
  language: string;
  engines: Array<SpeechEngineItem>;
...
export type TaskStatus =
  "pending" | "running" | "waiting" | "needsYou" | "paused" | "done" | "failed" | "cancelled" | "interrupted";

export type OnError =
  { policy: "stop" } | { policy: "continue" } | { policy: "retry"; times: number } | { policy: "ask" };

export type WindowEvent = "opened" | "closed";

export type WatchSpec =
  | { type: "time"; atMs: number }
  | { type: "processExit"; pid: number | null; name: string | null }
  | { type: "folder"; path: string; pattern: string | null }
  | { type: "download"; folder: string | null }
  | { type: "window"; app: string | null; title: string | null; event: WindowEvent };

export type Criterion =
  | { type: "commandSucceeds"; command: string; cwd: string | null }
  | { type: "fileExists"; path: string }
  | { type: "windowExists"; title: string };

export type StepAction =
  | { type: "tool"; tool: string; args: unknown }
  | { type: "watch"; watch: WatchSpec }
  | { type: "say"; text: string }
  | { type: "ask"; prompt: string; profile: string | null }
  | { type: "decide"; question: string; options: Array<string>; profile: string | null }
  | { type: "agent"; prompt: string; agent: string | null; cwd: string | null }
  | { type: "verify"; criterion: Criterion }
...
export type Trigger =
  | { type: "phrase"; phrases: Array<string>; lang: string }
  | { type: "hotkey"; chord: string }
  | { type: "manual" }
  | { type: "schedule"; cron: string; tz?: string | null }
  | { type: "event"; event: RoutineEvent };

export type RoutineEvent =
  | { kind: "appLaunched"; app: string }
  | { kind: "usbDevice"; name?: string | null }
  | { kind: "network"; name: string }
  | { kind: "timeOfDay"; time: string; days: Array<number> }
  | { kind: "idle"; minutes: number }
  | { kind: "return"; awayMinutes: number }
  | { kind: "batteryLow"; percent: number }
  | { kind: "afterRoutine"; routine: string };
...
export type AgentsOverview = {
   ...
   */
  desktop: Array<DesktopAiItem>;
  sessions: Array<AgentSessionItem>;
};
...
export type MemorySuggestionView = {
  id: number;
  text: string;
  reason: string | null;
  workspace: string | null;
...
export type MemoryOverview = {
   ...
   */
  root: string;
  notes: Array<MemoryNoteView>;
  tags: Array<MemoryTagView>;
  folders: Array<MemoryFolderView>;
  suggestions: Array<MemorySuggestionView>;
  /**
...
export type ComputerUse = {
   ...
   */
  "pause-on-mouse": boolean;
  speed: CuSpeed;
  "max-steps": number;
  /**
   * Most estimated cost per task, in US cents.
   */
  "max-cost-cents": number;
